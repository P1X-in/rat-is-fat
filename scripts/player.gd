...
func check_level_exit():
    ...
    for exit in self.bag.game_state.current_room.exits:
        exit_area = self.bag.room_loader.translate_position(Vector2(exit[0] + self.bag.room_loader.side_offset, exit[1]  + self.bag.room_loader.top_offset)) + Vector2(16, 0)
